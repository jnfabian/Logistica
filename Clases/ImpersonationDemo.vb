...
Public Class ImpersonationDemo
    ...
    Public Overloads Shared Sub Main()
        ...
        Try
            ...
            Dim userName, domainName, clave As String

            ' Get the user token for the specified user, domain, and password using the  
            ' unmanaged LogonUser method.   
            ' The local machine name can be used for the domain name to impersonate a user on this machine.
            'Console.Write("Enter the name of a domain on which to log on: ")
            domainName = "editoraperu.msft" 'Console.ReadLine()

            'Console.Write("Enter the login of a user on {0} that you wish to impersonate: ", domainName)
            userName = "usrdigital" 'Console.ReadLine()
            clave = "cendoc@ep"
            ...
            Dim returnValue As Boolean = LogonUser(userName, domainName, clave, LOGON32_LOGON_INTERACTIVE, LOGON32_PROVIDER_DEFAULT, safeTokenHandle)
...
Public NotInheritable Class SafeTokenHandle
    ...
    Public Shared Function getImpersonate() As WindowsImpersonationContext
        Dim tokenHandle As New IntPtr(0)
        Dim impersonatedUser As WindowsImpersonationContext = Nothing
        '**************Metodo para la obtencion del usuario de windows**********
        'MsgBox(Environment.UserName & vbNewLine & Environment.UserDomainName)
        Try
            If LogonUser("usrdigital", "editoraperu.msft", "cendoc@ep", 2, 0, tokenHandle) Then
